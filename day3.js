...
console.log(uniqueMerge([1,2,3,2],[3,4,1,5])); 
// ------------------------------------------------------------
...
console.log(findFirstIndexDivisibleBy([2,7,9,10,12,15,22], 3, 5));
...
function normalizeNames(names) {
    ...
    for (let i = 0; i < names.length; i++) {
        ...
        if (name.length > 0) {
            let normalized = name[0].toUpperCase() + name.slice(1).toLowerCase();
            res.push(normalized);
        }
    }
    return res;
...
let rawItems = [
    ...
    { name: "SSD", stock: 0 },
    { name: "Keyboard", brand: "HP", stock: 5 },
    { }
];
let c = buildProductCatalog(rawItems);
for (let i = 0; i < c.length; i++) {
    console.log(
        c[i].label(), 
        "Available?", c[i].isAvailable()
    );
}
...
console.log(sumUntilLimit([6, 6, 6], 10));
...
function safeLogin(user, policy) {
    ...
    let lowerPassword = user.password.toLowerCase();
    let lowerBlockWord = policy.blockWord.toLowerCase();
    if (lowerPassword.includes(lowerBlockWord)) {
        return false;
    }
    
    return true;
}
console.log(safeLogin(user ={ email: "a@b.com", password: "He11oWorld" } , policy = {minLen: 8, mustIncludeNumber: true, blockWord: "password" })
  ,
  )
// ============================================================
